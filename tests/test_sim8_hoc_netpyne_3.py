...
def test_load_figure8control_hoc_and_run_imported_cell() -> None:
    """Run Figure8control.hoc, then run the equivalent protocol on the PC2B JSON cell."""
    ...
    try:
        ...
        cell_json = repo_root / "cells" / "PC2B.json"
        assert cell_json.is_file(), "cells/PC2B.json is missing."

        net_params = specs.NetParams()
        with cell_json.open("r", encoding="utf-8") as handle:
            net_params.cellParams["PC2B"] = specs.Dict(json.load(handle))

        netpyne_time, netpyne_voltage, netpyne_current = _run_imported_cell_simulation(net_params)
        assert netpyne_time, "PC2B.json NetPyNE cell simulation did not record time samples."
        assert len(netpyne_time) == len(netpyne_voltage), "PC2B.json time/voltage traces differ in length."
        assert len(netpyne_time) == len(netpyne_current), "PC2B.json time/current traces differ in length."
        _write_trace_csv(output_dir / "sim8_json_voltage.csv", netpyne_time, netpyne_voltage, "voltage_mv")
        _write_trace_csv(output_dir / "sim8_json_current.csv", netpyne_time, netpyne_current, "current_nA")
    finally:
        os.chdir(prev_dir)
