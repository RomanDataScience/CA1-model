...
def _plot_trace(output_path: Path, time_ms: Sequence[float], voltage_mv: Sequence[float], current_na: Sequence[float]) -> None:
    figure, axes = plt.subplots(2, 1, figsize=(12, 8), sharex=True)

    axes[0].plot(time_ms, voltage_mv, linewidth=1.0, color="#1f77b4")
    axes[0].set_ylabel("Voltage (mV)")
    axes[0].set_title("PC2B: Figure 8 Ramp Protocol")
...
def run_pc2b(cell_json: Path, output_dir: Path) -> tuple[Path, Path]:
    ...
    net_params.cellParams["PC2B"] = specs.Dict(cell_rule)
    ...
    if not getattr(sim.net, "cells", None):
        raise RuntimeError("NetPyNE did not instantiate the PC2B cell.")

    cell = sim.net.cells[0]
    if "soma" not in cell.secs:
        available = ", ".join(sorted(cell.secs.keys()))
        raise RuntimeError(f"Expected a 'soma' section in PC2B. Available sections: {available}")
    ...
    csv_path = output_dir / "PC2B_trace.csv"
    png_path = output_dir / "PC2B_trace.png"

    _write_trace_csv(csv_path, time_ms, voltage_mv, current_na)
    _plot_trace(png_path, time_ms, voltage_mv, current_na)
...
def main(argv: Sequence[str] | None = None) -> int:
    repo_root = Path(__file__).resolve().parents[1]
    default_cell_json = repo_root / "cells" / "PC2B.json"
    default_output_dir = Path(__file__).resolve().parent / "results"

    parser = argparse.ArgumentParser(
        description="Run the Figure 8 PC2B ramp protocol in NetPyNE using cells/PC2B.json."
    )
    parser.add_argument(
        "--cell-json",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Directory where PC2B_trace.csv and PC2B_trace.png will be written.",
    )
    args = parser.parse_args(argv)

    try:
        csv_path, png_path = run_pc2b(args.cell_json, args.output_dir)
    except Exception as exc:
        print(f"PC2B run failed: {exc}", file=sys.stderr)
        return 1
