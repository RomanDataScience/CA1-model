...
def _validated_ynorm_range(pop_name):
    pop_y_norm_ranges = getattr(cfg, 'popYNormRanges', {})
    if pop_name not in pop_y_norm_ranges:
        raise ValueError(f"Missing cfg.popYNormRanges entry for population '{pop_name}'.")

    ynorm_range = list(pop_y_norm_ranges[pop_name])
    if len(ynorm_range) != 2:
        raise ValueError(
            f"cfg.popYNormRanges['{pop_name}'] must contain exactly two values."
        )

    ynorm_min = float(ynorm_range[0])
    ynorm_max = float(ynorm_range[1])
    if not 0.0 <= ynorm_min <= ynorm_max <= 1.0:
        raise ValueError(
            f"cfg.popYNormRanges['{pop_name}'] must satisfy 0 <= min <= max <= 1."
        )

    return [ynorm_min, ynorm_max]


def _positioned_population(pop_name, **pop_params):
    params = dict(pop_params)
    params['ynormRange'] = _validated_ynorm_range(pop_name)
    return params


# src_network diverges from src/ here: define an explicit CA1 depth axis so the
# added ynormRange bands map onto laminar position instead of a single-cell layout.
netParams.sizeX = float(getattr(cfg, 'netSizeX', 100.0))
netParams.sizeY = float(getattr(cfg, 'netSizeY', 1000.0))
netParams.sizeZ = float(getattr(cfg, 'netSizeZ', 100.0))
...
# Populations
# -----------------------------------------------------------------------------
# src_network diverges from src/ here: resident CA1 populations are no longer
# single representative cells. They are 10-cell ensembles separated by layer.
netParams.popParams['PC2B'] = _positioned_population('PC2B', cellType='PC2B', numCells=cfg.PYR)
netParams.popParams['OLM'] = _positioned_population('OLM', cellType='OLM', numCells=cfg.OLM)
netParams.popParams['VIP'] = _positioned_population('VIP', cellType='BilashVIP', numCells=cfg.VIP)

# src_network diverges from src/ here too: the external drives also receive
# laminar proxy bands so rasters and spatial ordering reflect their CA1 targets.
netParams.popParams['SC'] = _positioned_population(
    'SC',
    cellModel='VecStim',
    numCells=cfg.SC,
    spkTimes=cfg.thetaSpikeTimes,
)
netParams.popParams['PP'] = _positioned_population(
    'PP',
    cellModel='VecStim',
    numCells=cfg.PP,
    spkTimes=cfg.thetaSpikeTimes,
)
netParams.popParams['MS'] = _positioned_population(
    'MS',
    cellModel='VecStim',
    numCells=cfg.nMS,
    spkTimes=cfg.MS_train,
)
