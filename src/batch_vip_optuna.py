...
try:
    from batchtk.utils import expand_path as _batchtk_expand_path

    def expand_path(path, create_dirs=False):
        return _batchtk_expand_path(str(path), create_dirs=create_dirs)
except ImportError:
    def expand_path(path, create_dirs=False):
...
def main():
    results = optuna_search(
        ...
        interval=10,
        project_path=cwd,
        output_path=expand_path(REPO_ROOT / "batch_runs" / study_label, create_dirs=True),
    )
    return results
